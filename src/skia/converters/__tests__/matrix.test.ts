...
describe('toSkMatrix', () => {
  ...
  it('real PIXI.Matrix.scale().translate() composition', () => {
    // Pixi.translate прибавляет к tx/ty: после scale(2,3) → a=2,d=3; translate(10,20) → tx=10,ty=20
  ...
  it('composition rotate∘scale via real Pixi API: fields do not leak into wrong positions', () => {
    ...
    // sk = [a, c, tx, b, d, ty, 0,0,1] — сверяем каждую позицию с полем Pixi
